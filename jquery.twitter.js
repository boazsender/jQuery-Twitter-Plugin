;(function ($) {
  ...
  var opts = {
        ...
        exclude : '',      // Strings to exclude <-- not in twitter api, done in plugin
      ...
      hashtags  = function(str) {
        return str.replace(/[#]+[A-Za-z0-9-_]+/ig, function(tag) {
          ...
          return tag.link('http://search.twitter.com/search?q='+tagLink);
        });
      };
  ...
  $.fn.twitter = function (options) {
    ...
    exlude = options.exclude ? options.exclude : '';
    ...
    delete options.notFoundText;
    
    /*NOTE deal with exclusions*/
    
    // Check that options is an object, and merge it with the defaults, otherwise, use the options as a from username
